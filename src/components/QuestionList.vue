  ...
  <div class="max-w-sm mx-auto px-10 sm:px-4">
    ...
    <div v-else>
      ...
      <button
        v-if="!isResult"
        v-on:click="submit"
        class="px-10 bg-gray-800 hover:bg-gray-700 text-gray-200 px-4 py-2 rounded-md"
      >
        Save
      </button>
      ...
      <div
        v-if="isResult"
        class="mx-auto font-bold text-gray-700 rounded-full bg-green-300 flex items-center justify-center font-mono w-64 h-64 text-5xl border-4 border-green-400"
      >
        Total<br />{{ totalCorrectAnswer }} / {{ questionsResult.length }}
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    submit() {
      if (
        ...
        this.questions.length == this.questionsResult.length
      ) {
        this.isResult = true;
        this.displayResult();
      } else {
